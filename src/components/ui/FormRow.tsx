...
export const SectionTitle = ({ label, count, buttons, style }: SectionTitleProps) => (
  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 14, ...style }}>
    <div style={{ display: 'inline-block', fontSize: 14, fontWeight: 700, color: '#111111', paddingBottom: 5, borderBottom: `2px solid ${colors.primary}`, minWidth: 60 }}>
      {label}
      {count != null && (
        <span style={{ fontSize: 12, color: colors.textLight, fontWeight: 400, marginLeft: 6 }}>총 {count}건</span>
      )}
    </div>
...
export const PanelFooter = ({ onCancel, onSave, saveLabel = '저장', extraLeft }: PanelFooterProps) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, marginTop: 8, borderTop: `1px solid ${colors.border}` }}>
    <div>{extraLeft}</div>
