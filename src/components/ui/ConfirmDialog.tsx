...
export function ConfirmDialog({ open, title, msg, onOk, onCancel, okLabel = '확인', danger = true }: ConfirmDialogProps) {
  if (!open) return null;
...
export function UnsavedConfirmDialog({ open, onDiscard, onSave }: UnsavedConfirmDialogProps) {
  if (!open) return null;
