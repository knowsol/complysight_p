    ...
    if (readOnly) {
      return (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {selectedResources.length === 0 ? <span style={{ fontSize: 12, color: colors.textLight }}>선택된 자원이 없습니다.</span> : null}
          {selectedResources.map((res) => (
            <span key={res.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 12, background: colors.primaryLight, color: colors.primary, fontSize: 12, fontWeight: 600 }}>
              {res.nm}
              <span style={{ fontWeight: 400, color: colors.textSecondary }}>{res.mid}</span>
            </span>
          ))}
        </div>
      );
      ...
      <>
        ...
        <div style={{ border: `1px solid ${colors.border}`, borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ maxHeight: 180, overflowY: 'auto' }}>
            {available.length === 0 ? <div style={{ padding: 14, textAlign: 'center', fontSize: 12, color: colors.textLight }}>추가 가능한 자원이 없습니다.</div> : null}
            {available.map((res) => (
              <div
                key={res.id}
                onClick={() => setField('resources', [...form.resources, res.id])}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderBottom: `1px solid ${colors.border}`, cursor: 'pointer' }}
                onMouseEnter={(e) => {
              ...
              >
                <span style={{ color: colors.primary, fontWeight: 700, fontSize: 15 }}>+</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: colors.text }}>{res.nm}</div>
                  <div style={{ fontSize: 12, color: colors.textLight }}>{[res.sysNm, res.mid, res.ip || '—'].join(' · ')}</div>
                </div>
              </div>
            ))}
          </div>
    ...
    return (
      <div style={{ border: '2px dashed #19973C', borderRadius: 10, marginTop: 20, background: '#fff', padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: colors.textHeading }}>점검 결과</span>
        ...
        <div style={{ borderTop: `1px solid ${colors.border}`, marginBottom: 0 }} />

        {hasResult ? (
          <>
            <div style={{ fontSize: 14, fontWeight: 500, color: colors.textHeading, margin: '12px 0 10px', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{form.resultContent || '—'}</div>
            <div style={{ paddingTop: 10, borderTop: `1px solid ${colors.border}`, display: 'flex', flexWrap: 'wrap', gap: '4px 20px' }}>
              {[
            ...
            <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${colors.border}` }}>
              <div style={{ fontSize: 11, color: colors.textLight, marginBottom: 6 }}>점검보고서 첨부파일</div>
              {form.reportFile ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', border: `1px solid ${colors.border}`, borderRadius: 6, background: '#F9FAFC' }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={colors.primary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                    <polyline points="14 2 14 8 20 8" />
                  </svg>
                  <span style={{ flex: 1, fontSize: 12, color: colors.text, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{form.reportFile.name}</span>
                  <span style={{ fontSize: 12, color: colors.primary, cursor: 'pointer', fontWeight: 600, padding: '2px 8px', border: `1px solid ${colors.primary}`, borderRadius: 4 }}>다운로드</span>
                </div>
              ) : (
          ...
          <div style={{ padding: '20px 0', textAlign: 'center', fontSize: 13, color: colors.textLight }}>보고된 결과가 없습니다.</div>
        )}
      </div>
    );
  };
      ...
      >
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          ...
          <FormRow label="점검 제목" required>
            <FormInput value={form.title} onChange={(e) => setField('title', e.target.value)} placeholder="특별점검 제목을 입력하세요" readOnly={readOnly} style={errors.title ? { ...roStyle, borderColor: colors.red } : roStyle} />
            {errors.title ? <div style={{ fontSize: 11, color: colors.red, marginTop: 3 }}>{errors.title}</div> : null}
          </FormRow>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <FormRow label="점검 종류" required style={{ flex: 1 }}>
              ...
              {errors.kind ? <div style={{ fontSize: 11, color: colors.red, marginTop: 3 }}>{errors.kind}</div> : null}
            </FormRow>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <FormRow label="점검기한" required style={{ flex: 1 }}>
              ...
              {errors.due ? <div style={{ fontSize: 11, color: colors.red, marginTop: 3 }}>{errors.due}</div> : null}
            </FormRow>
            <FormRow label="점검자" required style={{ flex: 2, position: 'relative' }}>
              {readOnly ? (
                ...
                <div style={{ position: 'relative' }}>
                  <FormInput
                    ...
                    style={errors.insp ? { paddingRight: 28, borderColor: colors.red } : { paddingRight: 28 }}
                  />
                  {inspSearch || form.insp ? (
                    <span onClick={() => { setField('insp', ''); setInspSearch(''); }} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', cursor: 'pointer', color: colors.textLight, fontSize: 16, lineHeight: 1 }}>
                      ×
                    </span>
                  ) : null}
                  {inspSearch ? (
                    <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 200, background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 6, boxShadow: '0 4px 16px rgba(0,0,0,.1)', maxHeight: 180, overflowY: 'auto' }}>
                      {filteredUsers.length === 0 ? <div style={{ padding: '8px 12px', fontSize: 12, color: colors.textLight }}>검색 결과가 없습니다.</div> : null}
                      {filteredUsers.map((user) => (
                        <div
                          ...
                          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', cursor: 'pointer', borderBottom: `1px solid ${colors.border}` }}
                          onMouseEnter={(e) => {
                        ...
                        >
                          <span style={{ fontSize: 13, fontWeight: 500, flex: 1, color: colors.text }}>{user.userNm}</span>
                          <span style={{ fontSize: 11, color: colors.textLight }}>{user.userId}</span>
                          <span style={{ fontSize: 11, color: colors.textSecondary, background: '#F0F0F0', padding: '1px 6px', borderRadius: 8 }}>{user.userRole}</span>
                        </div>
                      ))}
                    </div>
                  ) : null}
              ...
              {errors.insp ? <div style={{ fontSize: 11, color: colors.red, marginTop: 3 }}>{errors.insp}</div> : null}
            </FormRow>
          </div>
          ...
          <FormRow label="점검계획서">
            {form.planFile ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', border: `1px solid ${colors.border}`, borderRadius: 6, background: '#F9FAFC' }}>
                ...
                <span style={{ flex: 1, fontSize: 12, color: colors.text, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {form.planFile.name}
                  {'size' in form.planFile && form.planFile.size ? <span style={{ fontSize: 12, color: colors.textLight, marginLeft: 6 }}>({(form.planFile.size / 1024 / 1024).toFixed(1)} MB)</span> : null}
                </span>
                {readOnly ? <span style={{ fontSize: 12, color: colors.primary, cursor: 'pointer', fontWeight: 600, padding: '2px 8px', border: `1px solid ${colors.primary}`, borderRadius: 4 }}>다운로드</span> : <span onClick={() => setField('planFile', null)} style={{ cursor: 'pointer', color: colors.textLight, fontSize: 18, lineHeight: 1 }}>×</span>}
              </div>
            ) : !readOnly ? (
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', border: `2px dashed ${colors.border}`, borderRadius: 6, cursor: 'pointer', transition: 'all .15s', background: '#fff' }}>
